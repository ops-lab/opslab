...
# export https_proxy=
pip=$(which pip)
pip_path=$(${pip} show opslab | grep 'Location' | awk '{print $2}')
opslab_path=$(pip_path)/opslab
...
config_uwsgi() {
    ...
    sed -i "s#CHDIR#${opslab_path}#g" ${uwsgi_config_file}
    uwsgi --ini ${uwsgi_config_file}
}

config_nginx() {
    cp ${ROOTPATH}/nginx.ini ${nginx_config_file}
    sed -i "s#CHDIR#${opslab_path}#g" ${nginx_config_file}
